        ...
        <>
          {error && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
              ...
              <button onClick={carregarDados} className="mt-3 text-sm text-red-600 hover:text-red-800 underline">Tentar novamente</button>
            </div>
          )}
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 transition-all hover:shadow-md">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900 mt-1">R$ {data?.vendasHoje.total.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                ...
                <div className="p-3 bg-green-50 rounded-xl"><ShoppingCart className="h-6 w-6 text-green-600" /></div>
              </div>
            </div>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 transition-all hover:shadow-md">
              <div className="flex items-center justify-between">
                ...
                <div className="p-3 bg-blue-50 rounded-xl"><BarChart3 className="h-6 w-6 text-blue-600" /></div>
              </div>
            </div>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 transition-all hover:shadow-md">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900 mt-1">R$ {data?.ticketMedioHoje.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                  <p className="text-xs text-gray-500 mt-1">Por transação no período</p>
                </div>
                <div className="p-3 bg-purple-50 rounded-xl"><DollarSign className="h-6 w-6 text-purple-600" /></div>
              </div>
            </div>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 transition-all hover:shadow-md">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900 mt-1">{(data?.itensVendidosHoje ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}</p>
                  <p className="text-xs text-gray-500 mt-1">Unidades totais</p>
                </div>
                <div className="p-3 bg-orange-50 rounded-xl"><Package className="h-6 w-6 text-orange-600" /></div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data?.vendasPorDia}>
                    ...
                    <XAxis dataKey="data" tick={{ fontSize: 12 }} tickFormatter={(val) => { const s = String(val); return /^\d{4}-\d{2}-\d{2}$/.test(s) ? s.split('-').slice(1).reverse().join('/') : s }} />
                    <YAxis tick={{ fontSize: 12 }} />
                    <Tooltip formatter={(value: number) => [`R$ ${value.toFixed(2)}`, 'Vendas']} labelFormatter={(label) => formatarDataBR(String(label))} />
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={data?.vendasPorPagamento} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="total" nameKey="forma">
                      {data?.vendasPorPagamento?.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}
                    </Pie>
                  ...
                  data.topProdutos.map((produto, index) => (
                    <div key={produto.nome} className="flex items-center justify-between">
                      <div className="flex items-center flex-1">
                        <div className="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-lg text-sm font-bold text-gray-500 mr-3">{index + 1}</div>
                        <div>
                      ...
                      <div className="text-right">
                        ...
                        <div className="w-24 bg-gray-100 h-1.5 rounded-full mt-1 overflow-hidden">
                          <div className="bg-blue-500 h-full rounded-full" style={{ width: `${(produto.total / (data.topProdutos[0]?.total || 1)) * 100}%` }}></div>
                        </div>
                      </div>
                    </div>
                  ))
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                    ...
                    {data?.vendasRecentes?.map((venda) => (
                      <tr key={venda.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-3 py-3">
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-gray-100 text-gray-600 uppercase">{venda.forma_pagamento}</span>
                        </td>
                        <td className="px-3 py-3 text-right font-bold text-gray-900">R$ {venda.valor_total.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="mt-4">
                <a href="/gestao/vendas" className="text-sm text-blue-600 hover:text-blue-800 font-medium flex items-center justify-center">
                  Ver todas as vendas <ArrowUpRight className="h-4 w-4 ml-1" />
                </a>
              </div>
            </div>
          </div>
